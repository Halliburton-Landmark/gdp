...
# Check for gdplogd and gdp_router first, then create directories, then run

# Exit if we use a previously unset variable
set -u

sourceDirectory=../..
gdplogd=../gdplogd/gdplogd
...
echo "Command to start gdp_routerL (cd $gdpRouterSource; python ./src/gdp_router.py -l routerLog.txt) &"
cd "$gdpRouterSource"
