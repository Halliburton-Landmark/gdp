...
/*
 * File descriptor types for scgi_register_fd_callbacks
 */
...
typedef enum
{
  ...
  SCGI_METHOD_GET, SCGI_METHOD_POST, SCGI_METHOD_PUT,
  SCGI_METHOD_DELETE, SCGI_METHOD_HEAD, SCGI_METHOD_OPTIONS
...
struct SCGI_PORT
{
  ...
  void *cbdata;			// passed for fd free callback
};
...
struct SCGI_DESC
{
  ...
  time_t lastuse;		//when this connection was last used
...
{									\
   ...
   {									\
      scgi_perror("scgilib: Out of RAM! Emergency shutdown." );	\
      abort();								\
   }									\
} while(0)
