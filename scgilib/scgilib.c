...
static void* (*scgi_fd_newfd_cb)(int fd, enum scgi_fd_type) = NULL;
static void (*scgi_fd_freefd_cb)(int fd, void *) = NULL;
...
void scgi_update_connections_port( scgi_port *p )
{
  ...
  for ( d = p->first_scgi_desc; d; d = d_next )
  {
    ...
    ||   d->lastuse + SCGI_KICK_IDLE_AFTER_X_SECS < now.tv_sec)
    {
...
void scgi_kill_socket( scgi_desc *d )
{
  ...
  if (scgi_fd_freefd_cb != NULL)
      (*scgi_fd_freefd_cb)(d->sock, d->cbdata);
  close( d->sock );
  free( d );
}
...
void scgi_answer_the_phone( scgi_port *p )
{
  ...
  if (scgi_fd_newfd_cb != NULL)
      d->cbdata = (*scgi_fd_newfd_cb)(caller, SCGI_FD_TYPE_DATA);
...
int resize_buffer( scgi_desc *d, char **buf )
{
  ...
  *size *= 2;
  if ( *size >= max )
  {
    // XXX should probably log something here
    scgi_kill_socket(d);
    return 0;
  }
...
void scgi_register_fd_callbacks(void *(*reg)(int fd, enum scgi_fd_type),
				void (*free)(int fd, void *))
{
    scgi_fd_newfd_cb = reg;
    scgi_fd_freefd_cb = free;
}
